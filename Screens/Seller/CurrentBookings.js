...
const CurrentBookings = ({ navigation }) => {
    ...
    const getOwnerBookings = async () => {
        try {
            ...
            const response = await fetch(`${HostName}product-services/get-owner-bookings`, {
                method: "GET",
                headers: {
    ...
    const handleCompleteBooking = async (bookingId) => {
        try {
            const jwtToken = await AsyncStorage.getItem("jwtToken");
            const response = await fetch(`${HostName}product-services/bookings/complete/${bookingId}`, {
                method: "POST",
                headers: {
                    'Authorization': `${jwtToken}`
                }
            ...
            setBookings(bookings.map(booking => booking._id === bookingId ? { ...booking, status: 'completed' } : booking));
        } catch (error) {
            Alert.alert("Error completing booking!", error.message || error);
        }
    }
